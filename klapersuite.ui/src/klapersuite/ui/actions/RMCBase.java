...
public class RMCBase implements IObjectActionDelegate {
	...
	public void run(IAction action) {
		Activator.getDefault().notifyLaunchingAnalysis("RMCBase");
		...
		try {
			...
			openInformation("Analysis with RMCBase failed due to invalid klaper model.");
			return;
		}
		...
		if(!resultFolder.exists()) {
			try {
				...
				openInformation("Analysis with RMCBase failed due to result directory creation failed.");
				return;
			}
		}
		try {
			...
			openInformation("Analysis with RMCBase failed due to dtmc model saving failed.");
			return;
		}
		...
		createFileFromInputStream(selectedFile, resultFolder, ".rmcbase.log.txt", "log result file", logContent);

		ReliabilityAnalysisResult result = reliabilityAnalysis.executeRMCBaseSolver();
		...
		createFileFromInputStream(selectedFile, resultFolder, ".rmcbase.result.txt", "result file", fileContent);

		ByteArrayInputStream logPostContent = new ByteArrayInputStream((
				result.getRmcModelDetails()
			).getBytes());					
		createFileFromInputStream(selectedFile, resultFolder, ".rmcbase.log.txt", "log result file", logPostContent);

		openInformation("Analysis with RMCBase is finished.");
	}
	...
	private void createFileFromInputStream(IResource selectedFile,
		...
		if(resultFolder.getFile(klaperfileName(selectedFile)+extensionFileName).exists()) {
			try {
				...
				openInformation("Analysis with RMCBase failed due to old "+ description +" deletion failed.");
				return;
			}
		}
		try {
			...
			openInformation("Analysis with RMCBase failed due to "+ description +" creation failed.");
			return;
		}
	}
