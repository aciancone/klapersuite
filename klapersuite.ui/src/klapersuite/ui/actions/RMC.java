...
public class RMC implements IObjectActionDelegate {
	...
	public void run(IAction action) {
		Activator.getDefault().notifyLaunchingAnalysis("RMC");
		...
		try {
			...
			openInformation("Analysis with RMC failed due to invalid klaper model.");
			return;
		}
		...
		if(!resultFolder.exists()) {
			try {
				...
				openInformation("Analysis with RMC failed due to result directory creation failed.");
				return;
			}
		}
		try {
			...
			openInformation("Analysis with RMC failed due to dtmc model saving failed.");
			return;
		}
		...
		createFileFromInputStream(selectedFile, resultFolder, ".rmc.log.txt", "log result file", logContent);
		...
		createFileFromInputStream(selectedFile, resultFolder, ".rmc.result.txt", "result file", fileContent);

		ByteArrayInputStream logPostContent = new ByteArrayInputStream((
				result.getRmcModelDetails()
			).getBytes());					
		createFileFromInputStream(selectedFile, resultFolder, ".rmc.log.txt", "log result file", logPostContent);

		openInformation("Analysis with RMC is finished.");
	}
	...
	private void createFileFromInputStream(IResource selectedFile,
		...
		if(resultFolder.getFile(klaperfileName(selectedFile)+extensionFileName).exists()) {
			try {
				...
				openInformation("Analysis with RMC failed due to old "+ description +" deletion failed.");
				return;
			}
		}
		try {
			...
			openInformation("Analysis with RMC failed due to "+ description +" creation failed.");
			return;
		}
	}
