...
public class JEPSolver implements Solver{
	...
	public void printCurrentAssign(){
		System.out.println(getCurrentAssign());
	}
	...
	public String getCurrentAssign() {
		...
		if(this.currentAssign==null || this.currentAssign.isEmpty()){
			result += "No valid assignment present."+ "\n";
		}else{
			result += "Iterations so far: "+this.numIterations+ "\n";
			for(String s : this.currentAssign.keySet()){
				result += s+"\t = "+this.currentAssign.get(s) + "\n";
			}
		}
		return result;
	}
